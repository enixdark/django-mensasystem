...
def home(request):
    ...
    #     if unicode(request.user.id) != request.user.id:
    #         raise Http404
    try:
        db = User.objects.get(pk=request.user.id)
    ...
    return render_to_response("index.html",{'user':request.user,'data':db,'time':d},context_instance=RequestContext(request))
